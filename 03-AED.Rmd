...
>**OJO** Capítulo en construcción va a cambiar para cuando sea la clase
...
donde $\ell \in \mathbb{N}$ con $\ell > 0$ y las $z_i$ sin pérdida de generalidad, son vectores columna  de longitud $n$ ($z_i = (z_{i,1}, z_{i,2}, \dots, z_{i,n})^T$). Una columna $z_{k}$ con $0 \leq k \leq \ell$ se le conoce como: 
  ...
  + *Categórica* si cada entrada de $z_{k}$ es una indicadora de pertenencia a algún conjunto (por ejemplo `Hombre / Mujer` ó `Ingresos Altos / Ingresos Medios / Ingresos Bajos`). Usualmente $z_{k}$ se representa con un caracter o con un entero. Una variable cateórica puede ser *lógica* si $z_{k}$ es un indicador que toma alguno de los dos valores: `r TRUE` ó `r FALSE`. 
  
  + *Ordinal*  Una variable ordinal es aquél $z_{k} \in \mathcal{C}$ donde sobre  $\mathcal{C}$ existe un orden total; es decir si $x,y,w\in z_{k}$ se tiene que:
...
### Definición [Estadístico observado]
Un estadístico (observado) es una función cuyo dominio es la matriz de datos observados $Z$ o una columna de la misma. Es decir, un estadístico es cualquier función de los datos (ver @wolfe2017intuitive). 
...
**8. Ajuste de modelo lineal (versión 1)**
